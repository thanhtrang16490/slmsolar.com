...
const TinTucPage = () => {
  ...
  const getData = async () => {
    ...
    const res = await fetch(`/tin-tuc/api?current=${meta.current}&pagesize=${meta.pageSize}`);
    if (!res.ok) {
  ...
  const handleOnChange = async (page: number, pageSize: number) => {
    ...
    const res = await fetch(`/tin-tuc/api?current=${page}&pagesize=${pageSize}`);
    if (!res.ok) {
      throw new Error(res.statusText);
    }
    const data = await res.json();
      ...
      <Row style={{ position: 'relative' }}>
        ...
        <Col style={{
          ...
          <div style={{ position: 'absolute', top: 0, left: '50%', translate: '-50% 0%', width: '100%', height: '600px' }}>
            <Typography.Title style={{ textAlign: 'left', margin: '30px 0 0px 20px', fontSize: '30px', fontWeight: '600' }} >Tin tức ngành điện</Typography.Title>
          </div>
        </Col>
      </Row>
